...
class Encoder(Model):
    def __init__(self):
        ...
        for name in ['pool1', 'pool2_pool', 'pool3_pool', 'conv1_relu'] : outputs.append( self.base_model.get_layer(name).output )        
...
class Decoder(Model):
    def __init__(self, decode_filters):
        ...
        self.conv3 = Conv2D(filters=1, kernel_size=3, strides=1, activation='sigmoid', padding='same', name='conv3')       
